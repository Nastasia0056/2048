...
public class Game {
    ...
    private boolean checkMoveLeft(){
        for (int i = 0; i < SIZE; i++) {
            for (int j = 1; j < SIZE; j++) {
                if (matrix[i][j-1] == matrix[i][j] && matrix[i][j] != 0 || matrix[i][j] != 0 && matrix[i][j-1] == 0)
                    return true;
            }
        }
        return false;
    ...
    private boolean checkMoveUp(){
        for (int i = 1; i < SIZE; i++) {
            for (int j = 0; j < SIZE; j++) {
                if (matrix[i-1][j] == matrix[i][j] && matrix[i][j] != 0 || matrix[i][j] != 0 && matrix[i-1][j] == 0)
                    return true;
            }
        }
        return false;
    ...
    public boolean move(DirectionType directionType){
        if (!checkMove() && !checkFull())
